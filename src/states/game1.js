...
export default class Game1 extends Phaser.State {
    ...
    create() {
        ...
        var colorArea = game.add.bitmapData(30,70);
        colorArea.ctx.beginPath();
        colorArea.ctx.rect(0,0,30,70);
        colorArea.ctx.fillStyle = '#ffc600';
        colorArea.ctx.fill();

        var floor = game.add.bitmapData(789,30);
        floor.ctx.beginPath();
        floor.ctx.rect(0,0,789,30);
        floor.ctx.fillStyle = '#088A08';
        floor.ctx.fill();

        var floor2 = game.add.bitmapData(300,30);
        floor2.ctx.beginPath();
        floor2.ctx.rect(0,0,200,30);
        floor2.ctx.fillStyle = '#088A08';
        floor2.ctx.fill();

        this.player = game.add.sprite(40, 10, colorArea);

        this.floor1 = game.add.sprite(0, 450, floor);
        this.floor2 = game.add.sprite(1050, 550, floor);

        game.physics.enable([this.player,this.floor1,this.floor2], Phaser.Physics.ARCADE);
    ...
    update () {
        ...
        game.physics.arcade.collide(this.player, this.floor1);
        game.physics.arcade.collide(this.player, this.floor2);
